...
namespace StoreComputer.Controllers
{
    public class AdminController : Controller
    {
        ...
        {
            var nhacungCap = db.NhaCungCaps.Where(p => p.tenNCC.Contains(SearchString)).ToList();
            return View(nhacungCap);
        }
        ...
        {
            var khachHang = db.KhachHangs.Where(p => p.tenKH.Contains(SearchString)).ToList();
        ...
        {
            var datHang = db.DatHangs.Where(p => p.tenKhachHang.Contains(searchString)).ToList();
            return View(datHang);
        }
        public ActionResult danhSachTuVan(int? page)
        {
            ...
            var listTV = db.TuVanKHs.OrderBy(x => x.maTV).ToPagedList(pageNumber, pageSize);

            return View(listTV);
        }
        public ActionResult suaTuVan(int? id)
        {
            var listTV = db.TuVanKHs.ToList();
            if (id == null)
            {
                return HttpNotFound();
            }
